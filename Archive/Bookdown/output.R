...
districts = read.csv2("~/SPL-Project/Dirstrict Data/List_of_districts.csv", header = TRUE, 
...
for (i in 1:dim(districts)[1]) {
    ...
    link = paste0("https://berlin.kauperts.de/Bezirke/", districts$District[i], "/Ortsteile/", 
...
# SUMMARY_AP - function that provides a summary for defined parameter: data -
# name of a data table column - parameter of the summary groupby - grouping
# parameter weight - weight for calculation of weighted averages thresshold -
# critical level of parameter
...
summary_ap = function(data, column, groupby, weight, thresshold) {
    ...
    for (i in unique(data[[groupby]])) {
        ...
        line$average = weighted.mean(data[data[[groupby]] == i, ][[column]], data[data[[groupby]] == 
            i, ][[weight]])
        line$hotspots = sum(ifelse(data[data[[groupby]] == i, ][[column]] > thresshold, 
            data[data[[groupby]] == i, ][[weight]], 0))
...
PM10 = summary_ap(ap15, "PM10-Belastung (berechnetes Jahresmittel [µg/m³]) 2015", 
    "Name des Straßenabschnittes", "Länge des Straßenabschnittes [m]", 20)
PM25 = summary_ap(ap15, "PM25-Belastung (berechnetes Jahresmittel [µg/m³]) 2015", 
    "Name des Straßenabschnittes", "Länge des Straßenabschnittes [m]", 20)


# redacting of a street name delete AS AK AD etc. at the beginning
PM10$str = sub("AK |AS |AD ", "", PM10$street)
PM25$str = sub("AK |AS |AD ", "", PM25$street)

# delete street notation at the end
PM10$str = sub("str.$|Straße|str$|-Straße|Strasse|-Strasse|straße|strasse|Str.$|-Str.$", 
    "", PM10$str)
PM25$str = sub("str.$|Straße|str$|-Straße|Strasse|-Strasse|straße|strasse|Str.$|-Str.$", 
    "", PM25$str)
...
# for streets that are in more than 1 district distribute numbers equally to all
# the districts
for (i in unique(PM25_merged_f$str)) {
    n = dim(PM25_merged_f[PM25_merged_f$str == i, ])[1]
    if (n > 1) {
        PM25_merged_f[PM25_merged_f$str == i, ][["length"]] = PM25_merged_f[PM25_merged_f$str == 
            i, ][["length"]]/n
        PM25_merged_f[PM25_merged_f$str == i, ][["hotspots"]] = PM25_merged_f[PM25_merged_f$str == 
            i, ][["hotspots"]]/n
        PM25_merged_f[PM25_merged_f$str == i, ][["n"]] = PM25_merged_f[PM25_merged_f$str == 
            i, ][["n"]]/n
    }
}
...
for (i in unique(PM10_merged_f$str)) {
    ...
    if (n > 1) {
        PM10_merged_f[PM10_merged_f$str == i, ][["length"]] = PM10_merged_f[PM10_merged_f$str == 
            i, ][["length"]]/n
        PM10_merged_f[PM10_merged_f$str == i, ][["hotspots"]] = PM10_merged_f[PM10_merged_f$str == 
            i, ][["hotspots"]]/n
        PM10_merged_f[PM10_merged_f$str == i, ][["n"]] = PM10_merged_f[PM10_merged_f$str == 
            i, ][["n"]]/n
    }
}
...
for (i in unique(districts$District)) {
    ap_ag$district = i
    ap_ag$PM10 = weighted.mean(PM10_merged_f[PM10_merged_f$district == i, ]$average, 
        PM10_merged_f[PM10_merged_f$district == i, ]$length)
    ap_ag$PM25 = weighted.mean(PM25_merged_f[PM25_merged_f$district == i, ]$average, 
        PM25_merged_f[PM25_merged_f$district == i, ]$length)
    ap = rbind(ap, ap_ag)
}

write.csv2(ap, "~/SPL-Project/Dirstrict Data/luftverschmutzung.csv", row.names = FALSE)
