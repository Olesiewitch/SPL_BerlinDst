  ...
  DataUnderURL = function(URL){
      # Obtain the data from the table stored under URL
      # 
      # Args:
      #   URL: URL adress. Under the URL the table which contains required data must
  ...
  # split each data table from the tblLst into individual data frames
 
  tblLst = lapply(lstUrl, DataUnderURL) %>%
  ...
  # decimel symbol, removes empty space between numbers and converst the data 
  # to numeric type
  # 
  # Args:
  #     column: name of the column in the dataframe which values suppose to be
  #     cleaned up and converted to numeric type
  #
  # Returns: 
  ...
  DistricToFullName = function (column){
     ...
     column[grepl("pan",column,ignore.case = TRUE)] = "Pankow" %>%
  ...
  return(column)
     ...
     Marz = (gsub("mar","Marzahn-Hellersdorf", Pan ))
  }
  ...
  spMbDt = data_frame(mbDst, jnrMb,snrMb)  # create a data frame
  colnames(spMbDt) = c("District","Senior Members of Sport Clubs",
                   "Junior Members of Sport Clubs")  # name the columns
  
 
  clbDst = c(sprtCl$X__9[7:18])  # chose sport club district name data
  clbNm = c(sprtCl$X__10[7:18])  # chose sport club number data
  
  clbsDt = data.frame(clbDst,clbNm)  # create a data frame
  ...
  # Function checkes based on the geografical coordinates, if bus stops in file
  # bsStp are in the chosen Berlin district and counts their number.  
  # 
  # Args:
  #     district: names of the polygon file whith the coordinates of the given 
  #     Berlin district.The lontitutude coordinates  must be the first column 
  #     of the file, the latitute the second column.  
  #
  # Returns: 
  # Number of bus stops in the given district or on its border ( in whichstops 
  # function 1 = in the polygon, 2 = on the border of the polygon , 
  # 0 = not in the polygon). 
    whichstops = point.in.polygon(bsStp$stop_lon, bsStp$stop_lat, district[, 1],
                                  district[, 2]) # check if bus stops in the polygon
    nrstops = length(whichstops[whichstops != 0])
  }
  
  bsLst = lapply(dstLst, NrofBusStops)  #apply NrofBusStops function to all the districts
  ...
  # Since every source of data spells the names of the Berlin districs 
  # diffrently (German signes vs no German signes, Short form vs Long form ect)
  # it is not possible to simply merge the data frame by district. Another 
  # approach is to merge data frames knowing that each of them contains district
  # data in the same order.
  
  DataByDistric = function (data.frame){
      # Function arrages the data frame alphaberically according to the district
      # name
      #
      # Args: 
      #     data.frame: the name of the data frame, which should be arranged. 
      #     The data frame must contain a column with name "District".
      #
      # Returns: 
      # Arranged according to the alphabetical order of districs data frame 
