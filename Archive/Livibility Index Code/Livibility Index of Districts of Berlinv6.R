...
livibility_index =data.frame(final_data$Nr, final_data$District,liv_spc,house_av,dens,hous_allow,trans_stops, bike_lines,e_car_charge,  park_spc, tourist,hotel_occ,restaurant_ha, sport_club,pupils,avg_grade, child_three_dc, child3_six_dc,nr_docs,act_sen, act_yth,traf_acc, str_cross,crime, soc_help, handi, employ, comp,tax_rev, bankr,green_spc, agr_res, trees)                             
colnames(livibility_index)= c("nr", "district","liv_spc", "house_av","dens","hous_allow","trans_stops","bike_lines","e_car_charge", "park_spc","tourist","hotel_occ","restaurant_ha", "sport_club", "pupils","avg_grade","child_three_dc","child3_six_dc", "nr_docs", "act_sen", " act_yth", "traf_acc", "str_cross","crime", "soc_help", "handi", "employ", "comp","tax_rev",
...
Physical_index_1=apply(livibility_index[34:38],1,sum)
Physical_index_2=apply(livibility_index[39:43], 1,sum)
Social_index=apply(livibility_index[44:60])
Economic_index
Enviromental_index

### TOTAL INDEX SCORE

Total_Index_Score=((Physical_index_1*phys1_weight)+(Physical_index_2*phys2_weight)+(Social_index*social_weight)+(Economic_index*economic_weight)+(Env_index*env_weight)
