...
```{r, echo=TRUE}
total_trees= trees[3:14,2] ## Selecting total number of trees 
```
...
English Translation : Trees per km 
Unit: Total
Year:2017
...
Source of data: <https://de.statista.com/statistik/daten/studie/13353/umfrage/hundehalter-und-hunde-in-berlin-nach-bezirken/>
Original name: Anzahl Hunde insg. 
English translation: Total number of dogs
Unit: Total number
Year: 2017

```{r, echo=TRUE}
dogs =read_excel("Dirstrict Data/statistic_id13353_hundehalter-und-hunde-im-bundesland-berlin-nach-bezirken-2016.xlsx",sheet = "Daten", range = "B5:D18")
dogs_data=dogs[-1, -2]## removing the unwanted variables 
colnames(dogs_data)= c("districts", "total number of dogs") ## translating the names of the columns
dogs_data_ord= dogs_data %>%arrange(districts) ## arranging the data according to the alfabetical order of the districs
```
### Open Green Space
...
```{r, echo=TRUE}
green=green <- read_excel("Dirstrict Data/statistic_id652716_oeffentliche-gruenflaechen-in-berlin-nach-bezirken-2017.xlsx", sheet = "Daten", range = "B5:C17")
...
```{r, echo=TRUE}
write.csv(statistik_data, file = "statistik_daten")
```