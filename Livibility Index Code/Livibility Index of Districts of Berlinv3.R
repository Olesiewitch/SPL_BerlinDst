...
### Arragning the data sets according to alphabetical order od districts and converting to numeric level 
...
berliner_ar=as.numeric(berliner_ar$Parking.Spaces)
...
statistik_arr=as.numeric(statistik_arr$trees.km)
#### please arrange the data accoriding to the alpabetical order of District and make sure the number are as.numeric

###Mergin data sets


final_data_frame = data.frame(stat_arr,fbinter_ar,berliner_ar,doctor_arr,kriminal_arr,statistik_arr) #### please add remaining columns & their names
colnames(final_data_frame)=c(colnames(stat_arr),colnames(fbinter_stadt_berlin_daten[2]),colnames(fbinter_stadt_berlin_daten[3]),colnames(berliner_verkehr_zahlen_daten[3]),colnames(nr_of_house_doctor[2]), colnames(kriminalitatsatlas_berlin_daten[3]),colnames(statistik_daten[4]))
...
#### Calculating each indicator (following the google spreadsheet link in Mark down)
...
livibility_index =data.frame(final_data$Nr, final_data$District,liv_spc,house_av,dens,hous_allow, bike_lines, park_spc, tourist,hotel_occ,sport_club,pupils,avg_grade, child_three_dc, child3_six_dc,nr_docs,act_sen, act_yth,traf_acc, str_cross,soc_help, handi, employ, comp,tax_rev, bankr,green_spc, agr_res, trees)                             
colnames(livibility_index)= c("nr", "district","liv_spc", "house_av","dens","hous_allow","bike_lines","park_spc","tourist","hotel_occ", "sport_club", "pupils","avg_grade","child_three_dc","child3_six_dc", "nr_docs", "act_sen", " act_yth", "traf_acc", "str_cross", "soc_help", "handi", "employ", "comp","tax_rev",
